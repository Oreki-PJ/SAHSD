...
def evaluate(model, pet, config, dataloader):
    ...
    if(config.task_type == 'multiclass'):
        metrics = compute_metrics_multi(all_labels, all_preds)
    else:
            ...
            if global_step % config.eval_every_steps == 0:
                for name, loader in [['dev', dev_loader], ['test', test_loader]]:
                    ...
                    if name == 'dev':
                        ...
                        os.makedirs(cfg.output_dir+f'{round(curr_score, 2)}/', exist_ok=True)
                        logger.info(f'Save model at {config.output_dir}')
                        tokenizer.save_pretrained(cfg.output_dir+f'{round(curr_score, 2)}/')
                        model.save_pretrained(cfg.output_dir+f'{round(curr_score, 2)}/')
